...
public class ProxyDroidService extends Service {
  ...
  final static String CMD_IPTABLES_REDIRECT_ADD_HTTP = 
		  "iptables -t nat -N REDSOCKS\n" 
				  + "iptables -t nat -A REDSOCKS -p tcp --dport 80 -j REDIRECT --to 8123\n"
				  + "iptables -t nat -A REDSOCKS -p tcp --dport 443 -j REDIRECT --to 8124\n"
				  + "iptables -t nat -A REDSOCKS -p tcp --dport 5228 -j REDIRECT --to 8124\n"
				  + "iptables -t nat -A OUTPUT -p tcp -j REDSOCKS\n"
				  + "iptables -t nat -A PREROUTING -i wlan0 -p tcp -j REDSOCKS\n";

  final static String CMD_IPTABLES_REDIRECT_REMOVE_HTTP= 
  ...
  private String host;
  private int port;
  private String tenantId; //for snae
  private String userName; //for snae
  ...
  public boolean handleCommand() {
    ...
    //start the snae session
    boolean ret = tmMgr.start(userName, tenantId);
    Log.i(TAG, "start session ret:" + ret);
  ...
  /**
   * Called when the activity is closed / service is stopped
   */
  @Override
